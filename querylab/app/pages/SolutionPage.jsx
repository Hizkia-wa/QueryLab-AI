  ...
  return (
    <div className="min-h-screen bg-white font-sans text-left selection:bg-indigo-100">
      ...
      <div className="max-w-6xl mx-auto px-6 pb-20">
        <div className="rounded-[3rem] bg-slate-900 text-white p-12 shadow-2xl overflow-hidden">
          <div className="grid lg:grid-cols-[1.5fr_1fr] gap-10 items-start">
            <div>
              <span className="inline-block mb-4 px-4 py-2 text-xs uppercase tracking-[0.3em] bg-indigo-500/20 text-indigo-100 rounded-full">
                AI Guided Error
              </span>
              <h2 className="text-4xl font-black mb-4">Latih Kepekaan SQL dengan Kesalahan Terarah</h2>
...
LIMIT 5; -- ❌ diubah: jumlahnya bukan 3
            ...
            <div className="w-full min-h-[520px] rounded-[2.5rem] bg-slate-800 p-8 shadow-xl">
              <div className="h-full flex flex-col justify-between gap-6 text-slate-100">
                <div>
                  <h3 className="text-3xl font-black mb-4">Buka Chat Mentor AI</h3>
                  <p className="text-slate-300 leading-relaxed mb-6">
                    Tombol chat shortcut tersedia di pojok kanan bawah halaman. Gunakan untuk membuka chat instan kapan saja di halaman mana pun.
                  </p>
                  <ul className="space-y-3 text-sm text-slate-300">
                    <li>• Kirim query SQL kamu.</li>
                    <li>• AI akan beri query yang salah plus komentar perbaikan.</li>
                    <li>• Ini fitur terpisah dari halaman solusi.</li>
                  </ul>
                </div>
                <div className="rounded-3xl bg-slate-900 p-5 border border-slate-700 text-slate-300">
                  <p className="text-sm mb-3 font-semibold">Panduan singkat:</p>
                  <pre className="bg-slate-800 p-4 rounded-3xl overflow-x-auto text-xs text-slate-200">
SELECT customer_id, MAX(total_amount) AS total_pembelian -- ❌ diubah: harusnya pakai agregasi untuk TOTAL, bukan nilai terbesar saja
FROM orders
WHERE status = 'paid'

...
export default function QueryLabAI() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: "bot", text: "Koneksi terputus. Cek API-mu!" }]);
    } finally {
      setLoading(false);
      setImage(null);
    }
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="absolute bottom-20 right-0 w-[350px] sm:w-[400px] max-h-[70vh] bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col"
          >
            ...
            <div className="p-4 bg-white border-t border-slate-100 shrink-0">
              ...
              <div className="flex gap-2">
                ...
                <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSend()} placeholder="Tanya SQL..." className="flex-1 bg-slate-100 rounded-xl px-4 py-2 text-sm outline-none" />
                <button onClick={handleSend} disabled={loading} className="p-2.5 bg-indigo-600 text-white rounded-xl"><Send size={20} /></button>
              </div>
            </div>
          </motion.div>
        )}
